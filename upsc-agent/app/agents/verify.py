    ...
    def verify_single_unit(unit: dict) -> tuple[dict | None, dict | None, int, int]:
        ...
        try:
            ...
            if not source_text:
                ...
                # Pass through without verification if no source text available
                verified = VerifiedUnit(
                    **{k: v for k, v in unit.items() if k in VerifiedUnit.model_fields},
                    verification_status=VerificationStatus.PARTIAL,
                    evidence_quality=0.5,
                    verified_at=datetime.now(),
                )
                return verified.model_dump(), None, 0, 0
            ...
            if not result:
                ...
                # Pass through with partial status
                verified = VerifiedUnit(
                    **{k: v for k, v in unit.items() if k in VerifiedUnit.model_fields},
                    verification_status=VerificationStatus.PARTIAL,
                    evidence_quality=0.5,
            ...
            # Use verified versions of fields if available
            verified = VerifiedUnit(
                unit_id=unit["unit_id"],
                ...
                evidence_map=[
                    ...
                    for e in result.get("evidence_map", unit.get("evidence_map", []))
                ...
                evidence_quality=float(result.get("evidence_quality", 0.5)),
                verified_at=datetime.now(),
            )
            return verified.model_dump(), None, llm.total_calls, llm.total_tokens
            ...
            err = ErrorRecord(
                phase="verification",
                article_id=unit.get("article_ids", [None])[0],
                error_type="verification_exception",
                message=str(e),
            ).model_dump()
            # Pass through with partial status on error
            verified = VerifiedUnit(
                **{k: v for k, v in unit.items() if k in VerifiedUnit.model_fields},
                verification_status=VerificationStatus.PARTIAL,
            ...
            return verified.model_dump(), err, 0, 0
