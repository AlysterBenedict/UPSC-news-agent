...
def _safe_build_evidence(raw_evidence: list, context_id: str) -> list[EvidenceItem]:
    """Build EvidenceItem list, skipping any items that fail validation."""
    safe = []
    for e in raw_evidence:
        if not isinstance(e, dict) or "claim" not in e:
            continue
        try:
            safe.append(EvidenceItem(**e))
    ...
    def analyze_single_cluster(cluster: dict) -> tuple[dict | None, dict | None, int, int]:
        ...
        try:
            ...
            unit = AnalysisUnit(
                ...
                evidence_map=_safe_build_evidence(result.get("evidence_map", []), cluster_id),
                ...
                upsc_relevance_score=_safe_float(result.get("upsc_relevance_score"), 5.0),
