...
2. Start with <h3>1. immediately. Output NOTHING before the first <h3> tag.
3. End with the last </p> tag. Output NOTHING after.
...
def _inject_unit_anchors(html: str, batch_units: list[dict]) -> str:
    """Inject per-article anchor IDs into brief-card headings for direct linking.
    ...
    table can link directly to individual articles, not just section headers.
    """
    h3_matches = list(re.finditer(r'<h3[^>]*>', html, re.IGNORECASE))

    # Work backwards to preserve character offsets
    ...
    def write_single_batch(task: dict) -> tuple[str, str | None, dict | None, int, int]:
        ...
        try:
            ...
            if html_fragment:
                ...
                # Inject per-article anchors for direct linking from revision table
                html_fragment = _inject_unit_anchors(html_fragment, batch)
                log.info(
                    "batch_written",
                    section=section_name,
                    batch=batch_range,
                ...
                return section_name, None, None, llm.total_calls, llm.total_tokens
            ...
            err = {
                ...
                "error_type": "write_exception",
                "message": f"Section {section_name} batch {batch_range}: {e}",
            }
            return section_name, None, err, 0, 0
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                errors.append({
                    ...
                    "message": f"Thread write exception: {e}",
                })
