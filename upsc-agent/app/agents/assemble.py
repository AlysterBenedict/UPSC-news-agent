...
def _build_revision_table_html(verified_units: list[dict], unit_to_section: dict[str, str]) -> str:
    ...
    for section_key in ordered_section_keys:
        ...
        for unit in sorted(units, key=lambda u: u.get("upsc_relevance_score", 0), reverse=True):
            ...
            rows.append(f"<tr><td>{topic_html}</td><td>{data_html}</td><td>{summary}</td></tr>")
...
<head>
    ...
    <style>
        @page {{ size: A4; margin: 1.5cm 1.5cm 2cm 1.5cm; @bottom-center {{ content: "Page " counter(page) " of " counter(pages); font-size: 8pt; color: #888; }} }}
        ...
        .gs-tag {{ font-size: 8.5pt; color: #555; margin-top: 2px; padding: 4px 8px; background: #f5f5f5; border-left: 3px solid #1a237e; }}
