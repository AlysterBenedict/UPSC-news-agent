...
CRITICAL RULE FOR POLITICS vs. POLICY (STRICTLY EXCLUDE POLITICAL DRAMA / POLITICS WITH NO POLICY VALUE):
UPSC preparation focuses strictly on administration, policy, governance, law, and international affairs. It has ZERO tolerance for political drama, electoral math, party bickering, election campaigns, and politician statements that lack administrative significance.
- You MUST classify any article focusing on political rivalry, electoral math, alliances, seat-sharing, candidate tickets, defection, internal party struggles, or political attacks as RELEVANT = FALSE.
- Substantive administrative policy significance is required: there must be a real government decision, legislation, implementation update, constitutional issue, or policy framework.
- Casual mentions of a policy, minister, or government department within a political fight does NOT make the article relevant. If the primary focus is political drama or mudslinging, reject it (RELEVANT = FALSE).
...
- Hyper-Specific Sensationalism / Local Crime / Regional Tragedies: regional tragedies, industrial accidents, or infrastructure issues that offer no structural or administrative takeaway (e.g. exact casualty counts, nationalities, or locations of a localized municipal fire).
...
- Political drama, fights, election campaigning, and political bickering:
  - All electoral calculations, seat sharing, candidate ticket distributions, candidate lists, constituency analyses, and election campaign coverage (rallies, speeches, slogans, promises with no administrative policy backing).
  ...
  - All generic political bickering, political mudslinging, and "war of words" ("Party A slams/attacks/mocks Party B" or "Opposition criticizes ruling party") that focus on political points rather than substantive policy analysis or administrative implementation details.
...
- "Local fire in warehouse kills two" → EXCLUDE (regional tragedy with no administrative takeaway)
- "Massive industrial chemical leak due to violation of environmental safety norms" → INCLUDE (governance/regulatory failure, administrative lens)
...
def classify_article_relevance(
    ...
    def classify_single(article: dict) -> tuple[str, dict, int, int]:
        ...
        try:
            ...
            log.error("classification_failed", article_id=article_id, error=str(e))
            # Default to relevant on failure — better to include than wrongly exclude
            return article_id, {"relevant": True, "reason": f"classification error: {str(e)[:50]}"}, 0, 0
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for idx, future in enumerate(as_completed(futures)):
            ...
            try:
                ...
                # Default to relevant on failure — better to include than wrongly exclude
                results[article_id] = {"relevant": True, "reason": f"thread error: {str(e)[:50]}"}
